...
\usepackage[
  ...
  unicode=true,
  pdftitle={A Proposed Solution to Erdős Problem 1002},
...
measure.  Erd\H{o}s asked whether the random variables
\[
 S_N(\alpha):=\sum_{k=1}^{N}\left(\frac12-\{k\alpha\}\right),
...
\]
is \(O(Kx^2)\).  When
\(\sqrt {y_{\rm CK}}<x\le K/(\log K)^{B_{\rm tail}}\), apply
part~(ii) of their theorem with the displayed substitution.  Its two
range hypotheses are checked separately:
\[
 x\le K\le y_{\rm CK},
 \qquad
 y_{\rm CK}<x^2\le x^2(\log x)^{B_{\rm CK}}
\]
for all sufficiently large \(K\); bounded remaining values are absorbed
in the constant.  The function denoted by \(\kappa\) in that theorem
...
applies to the resulting convergent symmetric series.  We may therefore
...
\(x=N^2\) it is larger than \(N\).  Apply
...
\]
Since \(w\) is Lipschitz and a conditional average lies between the
minimum and maximum of \(w\) on the cylinder,
...
expansions.  Hence the number of all finite cylinders whose terminal
...
Let \(T_N=\sum_iX_{N,i}\).  Since \(T_N\) is bounded by a rare count
of the form used in \eqref{eq:rare-factorial-bound}, for every fixed
\(m\),
\[
...
\(\mathscr R_\varepsilon\), the first factorial moment and
...
\]
respectively.  Kallenberg's mean-and-void criterion
\cite[Theorem~4.7]{Kallenberg}, applied to the countable dissecting
semiring \(\mathscr R_\varepsilon\) on this locally compact
second-countable space, therefore gives vague convergence to the
Poisson process with intensity \(\Lambda\).  Notice also that the mean
...
\(0\le n\le C_*L\) and value coordinate in a fixed compact interval
containing the value projection of \(\operatorname{supp}f\).  This
