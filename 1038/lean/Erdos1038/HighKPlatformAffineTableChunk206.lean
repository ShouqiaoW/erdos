...
import Erdos1038.HighKPlatformAffineCorrectionCertificateCap289
import Erdos1038.HighKPlatformAffineCornerPrefactorChunk206
import Erdos1038.HighKPlatformAffineCornerPenaltyChunk206
import Erdos1038.HighKPlatformAffineCornerSincGapChunk206
...
namespace Erdos1038.HighKPlatformAffineTableChunk206
...
theorem raw_206 : RawChecks (data ⟨206, by decide⟩) := by
  ...
  · apply uniformPositive_affineCorner_of_lower_components
      (prefactorLower := prefactorLower_206)
      (qCorrectionLower := cap289CorrectionLower)
      (rCorrectionLower := cap170CorrectionLower)
      (penaltyLower := penaltyLower_206)
      (sincGapLower := sincGapLower_206)
    · exact prefactor_206
    · apply uniformLower_of_evalLower
      change EvalLower (data ⟨206, by decide⟩).boxes
        (circleCorrectionLowerE scalarLogTerms scalarTrigDoubles
          scalarFourierTerms (.rat (289 / 100)) piE)
        cap289CorrectionLower
      exact cap289Correction (data ⟨206, by decide⟩)
    · apply uniformLower_of_evalLower
      change EvalLower (data ⟨206, by decide⟩).boxes
        (circleCorrectionLowerE scalarLogTerms scalarTrigDoubles
          scalarFourierTerms (.rat (170 / 100)) piE)
        cap170CorrectionLower
      exact cap170Correction (data ⟨206, by decide⟩)
    · exact penalty_206
    · exact sincGap_206
    · kernel_decide
  -- r_eval
...
end Erdos1038.HighKPlatformAffineTableChunk206
