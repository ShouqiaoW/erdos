...
import Erdos1038.HighKPlatformAffineCorrectionCertificateCap286
import Erdos1038.HighKPlatformAffineCornerPrefactorChunk236
import Erdos1038.HighKPlatformAffineCornerPenaltyChunk236
import Erdos1038.HighKPlatformAffineCornerSincGapChunk236
...
namespace Erdos1038.HighKPlatformAffineTableChunk236
...
theorem raw_236 : RawChecks (data ⟨236, by decide⟩) := by
  ...
  · apply uniformPositive_affineCorner_of_lower_components
      (prefactorLower := prefactorLower_236)
      (qCorrectionLower := cap286CorrectionLower)
      (rCorrectionLower := cap171CorrectionLower)
      (penaltyLower := penaltyLower_236)
      (sincGapLower := sincGapLower_236)
    · exact prefactor_236
    · apply uniformLower_of_evalLower
      change EvalLower (data ⟨236, by decide⟩).boxes
        (circleCorrectionLowerE scalarLogTerms scalarTrigDoubles
          scalarFourierTerms (.rat (286 / 100)) piE)
        cap286CorrectionLower
      exact cap286Correction (data ⟨236, by decide⟩)
    · apply uniformLower_of_evalLower
      change EvalLower (data ⟨236, by decide⟩).boxes
        (circleCorrectionLowerE scalarLogTerms scalarTrigDoubles
          scalarFourierTerms (.rat (171 / 100)) piE)
        cap171CorrectionLower
      exact cap171Correction (data ⟨236, by decide⟩)
    · exact penalty_236
    · exact sincGap_236
    · kernel_decide
  -- r_eval
...
end Erdos1038.HighKPlatformAffineTableChunk236
