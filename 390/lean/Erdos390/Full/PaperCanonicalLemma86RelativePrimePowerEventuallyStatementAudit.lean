...
/-!
# Expanded statement audit: relative prime-power clause of Lemma 8.6

This declaration repeats the complete public type.  It checks that the
nonvanishing constant precedes the prime cutoff and tilt box, that the two
mesh parameters remain independent, that both remainder limits are exposed,
and that the geometry, regression, coefficient, and relative-power clauses
all use one exact finite nuisance-Schur equivalence.
-/
...
theorem expanded_exists_paperFineMesh_cutoff_eventually_canonical_lemma86_relativePrimePower
