...
/-!
# Expanded statement audit: closed canonical Proposition 8.7

This declaration deliberately expands both public proposition abbreviations.
It exposes the paper-order choices, the local-on-active-support frozen ledger,
and every path and endpoint conclusion without an analytic call-site premise.
-/
...
theorem expanded_canonical_proposition87
    (cMesh : ℝ)
                  ...
                  ∀ t ∈ Icc (0 : ℝ) 1,
                    (∑ x : Fixed ⊕ B.sampleData.Sample,
                      B.combinedWeight fixedWeight (path t) x) =
                        (quota : ℝ) := by
  simpa only [CanonicalProposition87Statement,
    HasPhysicallyCenteredFixedPartitionFit] using
      canonical_proposition87 cMesh I U Cprom Cbank ledger
