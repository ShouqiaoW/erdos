...
/-!
# Statement audit for source-first ledger and post-bridge assembly

This audit freezes the complete expanded composition interface: the literal
source hypotheses, the exact Section 8 ledger and synchronization, and all
twelve pointwise post-height bridge outputs.
-/
...
example
  ...
  exact
    BankPaperRealization.exists_bankPaperCanonicalSectionNinePostHeight_sourceFirstLedgerPostBridgeAssembly
      M hc hdeltaStar hdeltaStarUpper hW hbetaAct hdelta F source Hsource
        cSource E sourceCellMargin hcSource hE hElarge hsourceLower Hready
