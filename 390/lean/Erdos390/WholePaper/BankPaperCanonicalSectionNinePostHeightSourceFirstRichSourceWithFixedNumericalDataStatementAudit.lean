...
/-!
# Statement audit for the fixed-numerical-data rich source

This audit freezes the complete strict-green interface after the pre-mesh
numerical witnesses have been fixed.  The displayed signature contains the
literal combined-charge terminal input, fixed mass coefficients, exponent,
source-margin equality, exponent inequality, and two-sided mass event.

The conclusion expands the dependent rich source, thin projection, exact
source event, post-ledger readiness event, and canonical guard geometry.
There is no unused source-margin-positivity premise and no named conclusion
contract.
-/
  ...
  exact
    BankPaperRealization.exists_bankPaperCanonicalSectionNinePostHeight_sourceFirstRichSourceWithFixedNumericalData
      hc hdeltaStar hW hbetaProt hbetaAct hbetaUpper hKlarge hprefix
        F hterminal cSource cUpper E sourceCellMargin hcSource hcUpper hE
        hsourceCellMarginEq hElarge Hmass M hdelta
