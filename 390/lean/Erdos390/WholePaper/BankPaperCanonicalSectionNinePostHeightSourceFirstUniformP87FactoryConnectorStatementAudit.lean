...
/-!
# Statement audit for the source-first uniform Proposition 8.7 factory

The first example unfolds the cutoff-first adapter statement, the
specialized factory, its eventual callback, and the exact one-index
Proposition 8.7 field literally.  The second assigns the production
theorem directly to the same complete expansion.

Thus the audited signature visibly selects `meshTol` and `W0` before the
final width, active-mass family, and particular permitted mesh, without
introducing a selector or Post-Hfit conclusion contract.
-/
