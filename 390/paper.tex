...
one- or two-mark band operator used below is
\begin{equation}
...
Every cell has \(\Theta_W(n)\) candidates, and (i) makes the probability
of a single candidate \(O_{B,W}(1/n)\).  Deleting the guards therefore
changes the normalizing constant by \(O_{B,W}(yL^A/n)\).  For a
one-mark row, a deleted integer contributes at most \(O(yL)\) after the
prime-row normalization; for a two-mark operator it contributes at most
\(O(yL^2)\), since \(v_p(m)\leq L/\log2\) and
\(\sum_qv_q(m)\leq L/\log W\).  Multiplication by the deleted mass and
allowing one further logarithm for normalization gives the bound in
\eqref{eq:marked-guard-relative-low-error}.  Finally
\(y^2L^{A+3}/n=n^{-5/9}L^{A+3}\), which remains \(o(1/\log L)\).
\end{proof}
...
Choose \(W\) sufficiently large and then \(\delta+\eta\) sufficiently
small.  There are constants \(\kappa>0\) and \(C_W<\infty\), independent
of the permitted regular mesh and of the radius \(B\) of a subsequently
fixed compact effective-tilt box, with the following quantifiers: for
every fixed \(B\) and every fixed permitted mesh \(\mathcal P\), there
is \(n_0=n_0(B,W,\mathcal P)\) such that the conclusions below hold for
all \(n\geq n_0\).  No common threshold over arbitrarily fine meshes or
over unbounded boxes is asserted.  Then \(\Gamma_\xi\) is positive
definite and
\begin{align}
...
\end{align}
The constants are uniform as \(t_0\downarrow0\) and over all regular
meshes satisfying \eqref{eq:smooth-regular-relative-mesh} with the stated
upper mesh size.
\end{lemma}

\begin{proof}
We first prove the continuum assertion with every object in the moving
low-cell argument explicit.  From \eqref{eq:smooth-kernel-product},
\begin{equation}
...
\end{equation}
indeed the harmonic average of \(s\) on \(I_0\) is exactly
\(\alpha_0^{(c)}\).  On a positive cell \(I_j\), the oscillation of
\(A^{(t_0)}J_{\mathcal P}b\) is at most
\(C\eta\|b\|_\infty\).  These are estimates in the ordinary supremum
norm, the norm used in the contradiction.
...
\end{equation}
The bound on \(\beta_\nu\) follows by pairing with
\(D_c\alpha^{(c)}\); the denominator is \(\gg1\) and the compressed
operators are uniformly bounded.  Formula \eqref{eq:smooth-low-row}
now gives
\begin{equation}
 |b_{\nu,0}|=O(\delta)+o(1).
...
\(b_{\nu,0}t/t_{0,\nu}\).  Its contribution to the full integral
equation is \(O(t_{0,\nu})\), uniformly in \(s\).  For the resulting
bounded function \(\widetilde f_\nu\),
\begin{equation}
 A\widetilde f_\nu=\beta_\nu t+O(\delta+\eta)+o(1)
...
gap uniformly as the low endpoint moves to zero.
...
with \(n\).  First absorb by a Neumann-series argument all \(O(1/W)\)
terms, including the squarefree diagonal correction and
\eqref{eq:smooth-power-row-transfer}.  If the remaining discrete inverse
failed, there would be gauge-fixed
\(b_k\), \(\|b_k\|_\infty=1\), and scalars \(\beta_k=O(1)\) such that
the unprojected normalized equation were
\begin{equation}
 \mathsf E_{n_k}\mathsf A^{\rm sf}_{n_k,\xi_k}J_{n_k}b_k
 =\beta_k\alpha+e_k,\qquad
 \|e_k\|_\infty\longrightarrow0.
 \label{eq:smooth-discrete-singular-sequence}
\end{equation}
Equations \eqref{eq:smooth-normalized-prime-row-error},
\eqref{eq:smooth-band-normalized-row-transfer}, and
\eqref{eq:smooth-collective-compact-quadrature} turn this into
\[
 B_{t_{0,k},\mathcal P}b_k
 =\beta_k\alpha^{(c)}+o_{\ell^\infty}(1),\qquad
 (\alpha^{(c)})^TD_cb_k=o(1).
\]
The first equality recovers every positive-cell diagonal value through
the multiplier \(h\geq c>0\), and
\eqref{eq:smooth-discrete-low-asymptotics} together with
\eqref{eq:smooth-low-row} recovers the low value.  Thus the explicit
linear extension used in
\eqref{eq:smooth-full-reconstructed-equation} produces a continuum
function with the continuum gauge.  The continuum inverse forces
\(\|b_k\|_\infty=o(1)\), the desired contradiction.  For the quadratic
statement, unit \(D\)-normalization and
\eqref{eq:smooth-positive-cell-data-convergence}--
\eqref{eq:smooth-low-cell-data-convergence} imply
\(\|J_{\mathcal P}b\|_{L^2(dt/t)}^2=1+o(1)\).  Only the newly appended
piece on \((0,t_0)\) has squared norm \(|b_0|^2/2=o(1)\); the norm of
the full extension is therefore \(1+o(1)\), not \(o(1)\).  Moreover,
\eqref{eq:smooth-compressed-form-identity}--
\eqref{eq:smooth-full-compressed-form-comparison} compare its full
quadratic form with the compressed continuum form.  The double
quadrature above also gives the required form convergence, despite the
growing low harmonic mass.  Indeed \(b^TDb=1\) implies
\(|b_0|\leq H_0^{-1/2}\).  The low-input quadrature error is \(o_n(1)\)
and is multiplied by \(b_0\); the normalized low-output error caused by
the fixed Mertens constant is \(O_W(H_0^{-1})\), whose contribution to
the form is
\(H_0|b_0|O_W(H_0^{-1})=O_W(H_0^{-1/2})\).  On the finitely many
positive cells, the \(D\)- and supremum norms are uniformly equivalent
for the fixed mesh.  Hence, uniformly for \(b^TDb=1\),
\begin{equation}
 b^TD\widehat B_nb-
 \langle\widetilde f,A\widetilde f\rangle_{\mathcal H}=o_n(1),
 \label{eq:smooth-discrete-full-form-comparison}
\end{equation}
where \(\widehat B_n\) denotes the squarefree arithmetic band operator
with its exact diagonal multiplier.  Thus both the norm and the
quadratic form needed for the Poincar\'e gap have been transferred.
...
chosen.  We choose \(W\) so that their \(1/W\) contributions are below
the continuum gap; after any fixed box is specified, the threshold for
\(n\) absorbs the two vanishing remainders.
...
\(O_W(1/L)\), and the physical cross row is given by the Stieltjes
...
therefore leaves the centered covariance span unchanged.  Then,
...
\end{equation}
for the fixed exponent \(A\) in that corollary.  Hence
...
The inverse assertion is a separate projected statement.  The continuum
inverse argument in
\eqref{eq:smooth-discrete-singular-sequence}--
\eqref{eq:smooth-power-row-transfer} gives the projected
\(\ell^\infty\)-inverse for the actual un-Schur band covariance
\(C_\xi\), after the two box-independent \(1/W\) errors have been
absorbed.  Apply
...
or the permitted fine mesh once \(W\) exceeds the fixed absorption
threshold.
...
not exist, there would be permitted meshes and vectors
\(q\in\mathcal G_c\) such that, after normalization,
\(\|q\|_{\sharp,c}=1\) while the right side of the projected compressed
equation is \(o(\alpha_j^{(c)})\) in every row.  As in
\eqref{eq:smooth-unprojected-compressed-equation}, for a bounded scalar
\(\beta\),
\begin{equation}
 (B_{t_0,\mathcal P}q)_j
 =\beta\alpha_j^{(c)}+o(\alpha_j^{(c)}).
 \label{eq:smooth-weighted-unprojected-equation}
\end{equation}
On a positive relative cell,
\(|t/\alpha_j^{(c)}-1|\ll\eta\).  On the low cell, harmonic centering
gives
\[
 \frac1{H_0^{(c)}}\int_{I_0}
   \left(\frac{t}{\alpha_0^{(c)}}-1\right)\frac{dt}{t}=0.
\]
Using \(h(t)=1+O(t)\) and \(|K(s,t)|\ll st\), replacement of the step
function by \eqref{eq:smooth-linear-cell-reconstruction} changes the low
averaged equation by \(O(w\alpha_0^{(c)})\), and changes a positive row,
after division by its center, by \(O(w)\).  More precisely, uniformly
over the permitted meshes,
\begin{equation}
 \max_j\frac{\left|
...
\eqref{eq:smooth-weighted-kernel-axis} gives
\(\operatorname{osc}_{I_0}G=O(\delta)\).  Moreover,
\[
...
side.  Pairing this full equation with \(t\) gives
\(\beta=O(w)+o(1)\).  The \(X_t\) inverse
\eqref{eq:PD-weighted-inverse} then puts the reconstruction within
\(O(w)+o(1)\) of \(\lambda t\).  Since
\((\alpha^{(c)})^TD_cq=0\), averaging
\(q_j/\alpha_j^{(c)}=\lambda+O(w)+o(1)\) against the positive weights
\(H_j^{(c)}(\alpha_j^{(c)})^2\) gives
\(\lambda=O(w)+o(1)\).  Once the previously fixed \(w=\delta+\eta\)
...
\end{equation}
The outer measure in this display is a probability measure, because
\(H_i\alpha_i=\sum_{p\in\mathcal P_i}t_p/p\).  Weighted Mertens
summation therefore converges uniformly against the continuous kernel
\(\varkappa\).  On the moving input cell the inner weighted mass is
\(H_0\alpha_0=\delta-t_0+o(1)\), while the prefactor
\(\alpha_0\to0\); on a positive input cell ordinary fixed-cell
quadrature applies.  This proves convergence of every kernel entry.
...
For the diagonal multiplier, use \(h(t)=1+O(t)\),
\eqref{eq:smooth-low-cell-data-convergence}, and ordinary positive-cell
quadrature.  The dimension is fixed after the mesh is chosen, so the
...
the low-cell value is \(\alpha_0\).  Finally the literal endpoints in
all four directions obey the common relative bound
\begin{equation}
...
\end{equation}
Indeed \(H_i\alpha_i=\sum_{p\in\mathcal P_i}t_p/p\), the low value is
\(\delta+o(1)\), and the crude bounds
\(\#\mathcal P_i\leq y\),
\(\sum_q|b_{j(q)}|\leq y\) dominate every \(L/n\) or \(L^2/n\)
endpoint in \eqref{eq:marked-four-tail-endpoint-ledger}.
...
\end{equation}
Thus the sharp discrete operator is a perturbation, in the sharp norm,
...
\(n\).  We finish by identifying the two varying quotient spaces
explicitly.  Put
\begin{align}
...
\end{align}
Equations \eqref{eq:smooth-sharp-projection-convergence},
\eqref{eq:smooth-sharp-main-quadrature},
\eqref{eq:smooth-sharp-squarefree-transfer},
\eqref{eq:smooth-sharp-guard-deletion},
\eqref{eq:smooth-sharp-squarefree-diagonal},
\eqref{eq:smooth-sharp-power-transfer}, and
\eqref{eq:smooth-sharp-head-physical-transfer} give the explicit
operator comparison
\begin{equation}
...
space by the displayed projections.  Since
\(\|\Pi_n-\Pi_c\|_{\infty\to\infty}=o_n(1)\), the restriction
\[
 U_n:=\Pi_n|_{\mathcal R_c}:\mathcal R_c\longrightarrow\mathcal R_n
\]
is an isomorphism for all sufficiently large \(n\), with
\(\|U_n\|+\|U_n^{-1}\|\le3\).  More precisely, under the ambient
coordinate identification,
\[
 \|U_n-I_{\mathcal R_c}\|_{\infty\to\infty}=o_n(1),\qquad
 \|U_n^{-1}-I_{\mathcal R_n}\|_{\infty\to\infty}=o_n(1).
\]
Since \(\mathcal T_c\) is uniformly bounded, direct multiplication gives
...
\end{equation}
Choose \(W\) so that the box-independent \(1/W\) term is smaller than
\((4C_\sharp^{(0)})^{-1}\), and only then fix the compact box and mesh
and take \(n\) large enough to absorb the remainder.  The Neumann
series and \eqref{eq:smooth-continuum-sharp-inverse} now give
\[
 \left\|(\mathcal T_{n,\xi}|_{\mathcal R_n})^{-1}\right\|_{\infty\to\infty}
 \le 2C_\sharp^{(0)}+o(1).
\]
Conjugating \eqref{eq:smooth-weighted-discrete-system} by \(S_n\)
therefore yields
\(\|q\|_\sharp\le C_\sharp w\), which is
...
Exact harmonic centering in each cell and
\eqref{eq:smooth-kernel-product} give, on every positive row,
\[
 \left|\left(D^{-1}
       \operatorname{Cov}_\xi(\Omega,S_g)\right)_j\right|
       \ll w\alpha_j.
\]
On the low row the product kernel gives the sharper calculation
\begin{align}
 \left|\left(D^{-1}
       \operatorname{Cov}_\xi(\Omega,S_g)\right)_0\right|
 &\ll \frac1{H_0}\sum_{p\in\mathcal P_0}
       \frac{t_p|g_p|}{p}
 +\alpha_0\sum_q\frac{t_q|g_q|}{q}
 +O_{B,W}(w/L)\notag\\
 &\ll w\alpha_0.
\label{eq:smooth-compensated-low-row}
\end{align}
The last error divided by \(w\alpha_0\) is
\(O_{B,W}(\log L/(\delta L))=o(1)\), so this is the required relative
low-row estimate.
The finite head and physical Schur subtraction is relative on every row,
not merely bounded in an unweighted norm.  Indeed the marked rows give
\(\|(U_\xi)_{i,*}\|\ll_{B,W}H_i/L\), the preceding score estimates give
\(\|\operatorname{Cov}_\xi(Z,S_g)\|\ll_{B,W}w\), and
\eqref{eq:smooth-joint-head-physical-gap} gives
\(\|\Gamma_\xi^{-1}\|\ll_W1\).  Therefore, uniformly on each fixed box,
\[
 \frac1{H_i}\left|
  \bigl(U_\xi\Gamma_\xi^{-1}
       \operatorname{Cov}_\xi(Z,S_g)\bigr)_i\right|
 \ll_{B,W}\frac wL=o_{n;B,W}(w\alpha_i),
\]
because \(\alpha_i\gg\delta\) on every positive row and
\(\alpha_0\asymp\delta/\log L\) on the moving low row.  The same
argument shows that the raw nuisance subtraction contributes only a
vanishing relative remainder to the right side of the regression
equation.
Thus the right side of \eqref{eq:smooth-block-explicit-regression} is
\(O(w\alpha_j)\) row by row.  Applying
...
\end{equation}
Using this bound in the same marked-row calculation shows that the
subsequent quotient correction also contributes only a vanishing
relative nuisance remainder.
Moreover \(a_Z=O_W(w)\), since \(\Gamma_\xi^{-1}=O_W(1)\) and the
centered head/physical covariance with
\(S_g-q^{\rm reg}\cdot\Omega\) is
\(O_W(w)\).  Equations \eqref{eq:smooth-g-size} and
...
\eqref{eq:smooth-full-reconstructed-equation}.  Cellwise Mertens
...
The finite nuisance Schur loss is, using the relative row estimate above,
\[
 \operatorname{Cov}_\xi\!\left(Z,
       S_g-q^{\rm reg}\!\cdot\Omega\right)^T\Gamma_\xi^{-1}
 \operatorname{Cov}_\xi\!\left(Z,
       S_g-q^{\rm reg}\!\cdot\Omega\right)
 =O_W(w^2/L^2)+o_{n;B,W,\mathcal P}(w^2).
\]
...
\[
 \left|\operatorname{Cov}_\xi(Z,S_g-q^{\rm reg}\cdot\Omega)\right|
 \ll_W w/L+o_n(w).
\]
Together with \(\Gamma_\xi^{-1}=O_W(1)\) and the exact physical removal
in \eqref{eq:smooth-physical-alpha-block-relation}, this gives
\begin{equation}
...
Finally, the squarefree diagonal term in the covariance with a fixed
marked row is \(O(w/p)\).  The off-diagonal term is also \(O(w/p)\),
because \(|K(t_p,t_q)|\ll t_pt_q\) and
\(\sum_qt_q|c_q|/q\ll w\).  Formula
\eqref{eq:smooth-JI-bound} and its transpose contribute
\(O(w/p^2+w/(pW))+\epsilon_{B,W}(n)O(w/p)\), and the remaining power
terms are smaller.  The nuisance regression contributes
\(O_W(w/(pL))\) by the marked head/physical rows.  This proves
...
By \Cref{lem:arithmetic-band-quotient}, the first term
\(A_\xi^{-1}v_{\mathscr Q}\) has bounded effective coefficients.
\Cref{lem:augmented-bridge} says, in the block notation just used, that
\[
 \sigma_\xi\asymp_Ww^2,\qquad
 \left|v_g-b_\xi^TA_\xi^{-1}v_{\mathscr Q}\right|\ll_Ww,
\]
and that the prime coefficients of \(A_\xi^{-1}b_\xi\) are
\(O(w\alpha_{j(p)})\), while its head/physical coefficients are
\(O_W(w)\).  Hence \eqref{eq:smooth-full-block-inverse} gives
\begin{equation}
...
\end{equation}
For completeness, the only target pairing in the numerator which is not
immediate from the displayed norms is also explicit.  If
\(q^{\rm reg}\) is the quotient part of \(A_\xi^{-1}b_\xi\), then
\[
 \left|(A_\xi^{-1}b_\xi)^Tv_{\mathscr Q}\right|
 =\left|\frac L{q_n}\sum_jq_j^{\rm reg}\Delta_j\right|
 \ll w\sum_jH_j\alpha_j\ll w.
\]
Here the \(Z\)-target is zero,
\(|q_j^{\rm reg}|\le C_{\sharp}w\alpha_j\), and
\(\sum_jH_j\alpha_j=\sum_{W<p\le y}t_p/p=O(1)\).
...
\eqref{eq:marked-tilted-physical-full}, and the third from
